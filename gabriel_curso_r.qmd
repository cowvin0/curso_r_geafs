...
```{r eval=TRUE}
meu_vetor2[2:4] <- c("Pedro", "Ataline")
meu_vetor2
```
...
meu_dataframe <- data.frame(
  pessoa = c("Batman", "Dr. Manhattan", "Simone"),
  idade = c(30, 43, 45),
  Sexo = factor(c("M", "M", "F"))
)
meu_dataframe
...
O pacote do tidyverse que serve para importação de banco de dados, é o `readr`. Por exemplo, podemos utilizar a função `read_csv` para ler um arquivo `.csv`. Veja um exemplo com dados de Vazão:

```{r eval=TRUE}
df <- read_csv("vazao.csv")
head(df)
```
...
df <- df |>
  ...
  mutate(Dia = as.double(str_replace(Dia, "Vazao", ""))) |>
  arrange(Data, Dia)
